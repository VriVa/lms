...
const CourseDetails = () => {
  ...
  const {allCourses,calculateRating,calculateChapterTime, calculateCourseDuration, calculateTotalNoOfLectures,currency}= useContext(AppContext)
...
const toggleSection = (index) => {
  ...
  return courseData ? (
    ...
    <p>{calculateRating(courseData)}</p>
        ...
        <p className='text-sm'>Course by <span className='text-blue-600 underline'>VriVa</span></p>
...
<div className={`overflow-hidden transition-all duration-300 ${openSection[index] ? 'max-h-96' : 'max-h-0'}`}>
  <ul className='list-disc mdpl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
    {chapter.chapterContent.map((lecture, i)=>(
      <li key={i} className='flex items-start gap-2 py-1'>
        <img src={assets.play_icon} alt="play icon" className='w-4 h-4 mt-1'/>
        <div className='flex items-center justify-between w-full text-gray-500 text-xs md:text-default'>
    ...
    <img className='w-3.5' src={assets.time_left_clock_icon} alt="time left clock icon " />
                ...
                <div className='flex items-center text-sm md:text-default gap-4 pt-2 md:pt-4 text-gray-500'>
                  ...
                  <img src={assets.lesson_icon} alt="lesson icon " />
                    <p>{calculateTotalNoOfLectures (courseData)} lessons</p>
                  </div>
                </div>
